...
//------------------------------------------------------------------------
/*!
\brief Report JPEG codec status when encode or decode is finish.
\par Note:
		JPEG_CODEC_PROCESS	: JPEG encode or decode is already in the processing.
		JPEG_CODEC_ERR	 	: JPEG encode or decode fail.\n
		JPEG_CODEC_AHB_ERR  : JPEG encode or decode fail.\n
		JPEG_CODEC_OK		: JPEG encode or encode pass.
\return flag
*/
uint8_t ubJPEG_ReportCodecStatus(void);
//------------------------------------------------------------------------
/*!
\brief Update quantization table to JPEG header
\param pJpgHdrAddr 		JPEG header memory address.
\param uwQtAddrOffset 	Quant table address offset.
\return(no)
*/
void JPEG_UpdateQuantTab2Header(uint8_t *pJpgHdrAddr, uint16_t uwQtAddrOffset);
//------------------------------------------------------------------------
/*!
