...
typedef enum
{
	BB_DISABLE_ALL_STA_WAKEUP 	= 0,
	BB_ENABLE_ALL_STA_WAKEUP 	= 1,
	BB_ENABLE_T2R_WAKEUP		= 2,
}SET_WOR_MODE;
...
\param ubTimingMd (input)   0:video first, 1:audio first, 2:video only
\return(No)
*/
void BB_Init(SET_SLOT_MODE tBB_SlotMode, uint8_t *pData, uint8_t ubStauts,uint8_t ubGIO1,uint8_t ubGIO2,uint8_t ubMode,uint8_t ubDetectRssi,uint8_t ubTimingMd);

/*!
\brief BB Dummy Initial
\param (No) 	
\par Note
	 Dummy Initial
\return (No)
*/
void BB_DummyInit(void);
//------------------------------------------------------------------------
/*!
\brief Set GPIO pin of RF interface.
\param ubCKO_Num 	RF CKO Pin
\param ubTxSW_Num	RF TX switch Pin
\return(No)
*/
void BB_SetRFGpioPinNum(uint8_t ubCKO_Num, uint8_t ubTxSW_Num);
//------------------------------------------------------------------------
/*!
...
*/
SET_BUF tBB_SendData(osMessageQId *Queue,SET_TYPE Typ,uint8_t *Addr,uint32_t ulLen,SET_TX_PATH tTxPath,uint8_t ubFrameTyp);
//------------------------------------------------------------------------
/*!
...
//------------------------------------------------------------------------
/*!
\brief Initail Baseband Variable,Set video/audio TX/RX buffer size 
\param ulBaseAddrs 	  (input) Basebnad Variable Start Address
\param ulBaseAddrs 	  (input) Basebnad Variable Start Address
\par Note [BB_VariableInit(uint32_t ulBaseAddrs) before set]
		(No)
\return (No) 
			
*/
void BB_SetVideoAudioBufSize(uint32_t ulVdoISize,uint32_t ulVdoPSize,uint32_t ulAdoSize);
//------------------------------------------------------------------------
/*!
...
//------------------------------------------------------------------------------
/*!
\brief  Get who wakeup device number
\return	0,1,2,3,0xF(wakeup success),0xFF(wakeup fail)
*/
uint8_t ubBB_GetWakeUpDeviceNumber(void);
//------------------------------------------------------------------------------
/*!
\brief  Get wakeup user define data (after use of [uint8_t BB_ConfirmWakeUpInf(void)]) 
\return	1 byte data 
*/
uint8_t ubBB_GetWakeUpUseDefineData(void);
//------------------------------------------------------------------------------
/*!
\brief  Send wakeup user define data (before use of [void BB_SetWakeUp(uint8_t ubStatus,uint8_t ubSta)])
\param  ubSet   1 bytes data
\return	(no)
*/
void BB_SendWakeUpUseDefineData(uint8_t ubSet);
//------------------------------------------------------------------------------
/*!
\brief  Set WOR mode
\param  SET_WOR_MODE (BB_DISABLE_ALL_STA_WAKEUP),(BB_ENABLE_ALL_STA_WAKEUP)   
\return	(no)
*/
void BB_SetWorMode(SET_WOR_MODE tMd);
//------------------------------------------------------------------------------
/*!
\brief 	Set wakeUp	
\param  ubStatus    0:Disable WakeUp, 1:Enable WakeUp
\param  ubSta    	[0,1,2,3,0xF(for VBM)][0xF(for BUC STA to wakeUp AP),0xFF(for BUC AP to wakeUp STA)]
\return	(no)
*/
...
/*!
\brief  AP only
\param  ubStatus = 1:Enable, 0:Disable
\param  ubCH = 1 ~ 14,
\return	0
*/
void BB_SetRxWifiScan(uint8_t ubStatus,uint8_t ubCH);
//------------------------------------------------------------------------------
void BB_SetWorkSta(uint8_t ubWorkSta);
EN_RETURN_STATUS tEN_Start(EN_STATUS tEnStatus,EN_SET_ROLE tRole,uint8_t *pData1,uint8_t *pData2); 
void BB_PairingStartOrEndReSetDefaulf(uint8_t ubStatus);
void BB_SetTRxTime(uint16_t uwTime);
void BB_SetDvrMode(uint8_t ubMod);	//set 1 DVR mode,set 0 other mode
void BB_SetRfPinMode(uint8_t ubMode);	//set RFSPI 1/RFSPI 2. 1 or 2
void BB_SetFastCmd(uint8_t *pData);
void BB_SetRxFastCmd(uint8_t ubNb,uint8_t *pData);
void BB_GetRxFastCmd(uint8_t ubNb,uint8_t *pData);
void BB_SetRfStopTx(uint8_t ubStatus);
void BB_SetSlotMode(SET_SLOT_MODE tSlotMode);
void BB_SetAccessReadVdoBufNum(uint8_t ubReadIframeBufNum,uint8_t ubReadPframeBufNum);
void BB_SetHopChNumber(uint8_t ubFlg);
