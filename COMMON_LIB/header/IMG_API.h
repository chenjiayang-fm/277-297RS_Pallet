...
#define IMG_DS3_Height						(64)   //!< IMG_DS3_Height
...
}IMG_DS_PAT_INFOR_TYP;						    //!< OSD Palette Information Structyure

/** \defgroup IMG_MODULE Image Control Modules
...
typedef enum
{
	IMG_SUCESS,						   //!<  Sucess Status
	IMG_FAIL						   //!<  Fail Status	
}IMG_BOOL_RESULT;

typedef enum
{
	IMG_DISABLE,					   //!<  Disable Status
	IMG_ENABLE						   //!<  Enable Status
...
typedef struct 
{
	struct H264_TASK *H264_Task;		//!<  Pointer of H264 Task Information Structure
	uint32_t InputSrcAddr;				//!<  Input Raw Data Memory Address (Must be mutiple of 8 byte)
	IMG_ENABLE_STATUS JPEGEnable;		//!<  JPEG ENABLE/DISABLE Status
	IMG_ENABLE_STATUS ScalingEnable;	//!<  Scalling Down ENABLE/DISABLE Status
}IMG_IMAGE_TASK;
...
typedef struct 
{
	IMG_ENABLE_STATUS STATUS;					//!< ENABLE/DISABLE Status
...
typedef enum
{
	IMG_DS_BUF_EMPTY,							//!< DS buffer empty status
	IMG_DS_BUF_CUR,								//!< DS buffer used by LCD controller
	IMG_DS_BUF_READY							//!< DS buffer ready status
}IMG_DS_BUF_STA_TYP;
...
typedef struct 
{
	uint32_t ulWidth;							//!(Origianl image width must be mutiple of 16 byte)
	uint32_t ulHeight;							//!(Origianl image height must be mutiple of 8 byte)
...
typedef struct 
{
	uint32_t ulWidth;							//!Image Width (must be mutiple of 16 byte)
	uint32_t ulHeight;							//!Image Height (must be mutiple of 8 byte)
...
//------------------------------------------------------------------------
/*!
\brief Set Image Control Thread Priority
\return(no)
\par [Example]
\code 
		IMG_SetPriority(osPriorityHigh);
\endcode
*/
void IMG_SetPriority(osPriority priority);
//------------------------------------------------------------------------
/*!
...
//------------------------------------------------------------------------
/*!
\brief Setup Cropping IP Parameter
...
//------------------------------------------------------------------------
/*!
	\brief 		Image 1 OSD Function
	\param[in]  DS_NUM		  DateStamp Number	
	\param[in]	pInfor		  DateStamp image 1 OSD information
	\param[in]	tMode		  Image 1 OSD updates method
	\return		IMG_SUCESS 	  DateStamp image 1 OSD Sucess!
	\return		IMG_FAIL DateStamp image 1 OSD Fail!
	\par [Example]
	\code
		 IMG_DS_IMG_INFO tInfor;
		 tIMG_DS_GetOsdImgInfor (DS_1, 1,DS_IMG1, IMG_DS_A, 1, &tInfor)
		 tIMG_DS_Img1 (&tInfor, OSD_UPDATE);
	\endcode
*/
...
/*!
	...
	\return		IMG_SUCESS 		  	 Get image imformation ok!
	\return		IMG_FAIL 			 Get image imformation fail!
	\par [Example]
	\code    
		 OSD_IMG_INFO tInfor;
...
//------------------------------------------------------------------------
/*!
\brief Date Stamp Size Setting
\param DS_NUM			Date stamp index
\param uwDS_HStart		Date stamp panel horizontal start point on image
\param uwDS_VStart 		Date stamp panel vertical start point on image
\param uwDS_HSize		Date stamp panel horizontal size
\param uwDS_VSize		Date stamp panel vertical size
\return		IMG_SUCESS 		  	 Set Date Stamp Size  ok!
\return		IMG_FAIL 			 Set Date Stamp Size  fail!
\par [Example]
\code 
		IMG_BOOL_RESULT = RESULT;
		RESULT = IMG_DSInit( 0, 0, 640, 480);
\endcode
*/
