...
typedef enum
{
    SF_STRAIGHT_WAIT,                   //! straight wait, in RTOS environment, will occupy CPU time to all lower threads
    SF_WAIT_IN_IDLE,                    //! in RTOS enviroment, will lower the current thread to idle priority (recommended)
    SF_WAIT_IN_LOWER_PRIO,              //! in RTOS enviroment, will lower 1 step priority of the current thread (recommended)
    SF_FIRE_AND_FORGET                  //! don't waiting for ready, just fire and forget, the ready will be wait in following Serial-Flash access
} SF_WaitRdyMethod_t;
...
typedef struct
{
	...
	SF_DmaDir_t         tDir;
	SF_WaitRdyMethod_t  tWaitRdyMethod;
...
\param ubWaitInIdle if 1, will set currently running thread to idle priority\n
                    while waiting for erase ready
\return(no)
*/
void SF_Erase(uint8_t ubEraseCmd, uint32_t ulStaAddr, uint32_t ulLength, uint8_t ubWaitInIdle);
//------------------------------------------------------------------------------
void SF_SetWpPin(uint8_t ubPin);
//------------------------------------------------------------------------------
/*!
\brief SF_GeneralRead function
\Examples:
	1.Read identification(RDID) at MX25L3233F
		SF_GeneralRead(0x9F, 0, 0, 0, 0, 3, pBuf); //! Buf[0] = 0xC2, Buf[1] = 0x20, Buf[2] = 0x16
	2.Read electronic manufacturer and device ID(REMS) at MX25L3233F
		SF_GeneralRead(0x90, 2, 1, 0x00, 0, 2, pBuf); //! Buf[0] = 0xC2, Buf[1] = 0x15
	3.Read serial flash discoverable parameter(SFDP) at MX25L3233F
		SF_GeneralRead(0x5A, 0, 3, 0x000000, 1, 4, pBuf); //! Buf[0]=0x53, Buf[1]=0x46, Buf[2]=0x44, Buf[3]=0x50
\return(no)
*/
void SF_GeneralRead(	uint8_t ubCommand,
						uint8_t ubDummyByteLengthHead,
						uint8_t ubAddrByteLength,
						uint32_t ulAddress,
						uint8_t ubDummyByteLengthTail,
						uint16_t usDataByteLength,
						uint8_t *pDataBufAddr );
//------------------------------------------------------------------------------
