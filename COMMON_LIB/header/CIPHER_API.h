...
typedef struct CIPHER_GENERIC_DMA_COPY_INFORMATION
{
	uint32_t ulAddr;
	uint32_t ulLineSz;
	uint32_t ulLineGapSz;
	uint32_t ulLineNum;
}CIPHER_GEN_DMA_INFO_t;
//------------------------------------------------------------------------
/*!
\brief Initial Cipher
\param WatingMode	cipher wating mode.
\return(no)
*/
...
//------------------------------------------------------------------------
/*!
	\brief 		DMA Copy Function
	\param[in] 	ulReadAddr 	Source address
	\param[in] 	ulWriteAddr Destination address
	\param[in] 	ulData_Len	Data length
	\par [Example]
	\code
		 CIPHER_DMACopy(0x300000, 0x500000, 2048);
	\endcode
*/
void CIPHER_DMACopy(uint32_t ulReadAddr, uint32_t ulWriteAddr, uint32_t ulData_Len);
//------------------------------------------------------------------------
/*!
	\brief 		DMA Copy Function
	\param[in] 	SrcInfo 	source information
	\param[in] 	DstInfo		destination information
*/
void CIPHER_DMAGenericCopy(CIPHER_GEN_DMA_INFO_t *SrcInfo, CIPHER_GEN_DMA_INFO_t *DstInfo);
//------------------------------------------------------------------------
/*!
