...
//	for(uint8_t i = 0;i < 4;i++)
//	{
//		tOsdImgInfo[i].uwXStart = X_Pos + X_Step*i;
//		tOsdImgInfo[i].uwYStart = 125;
//		tOSD_Img2(&tOsdImgInfo[i], OSD_QUEUE);
//	}
...
//				break;
	...
	//high light
	tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_AI_OFF_NOR, 4, &tOsdImgInfo[0]);
	tOsdImgInfo[2*tUI_CuSetting.ubDetectPeopleFlag[0] + 1].uwXStart = X_Pos;
	tOsdImgInfo[2*tUI_CuSetting.ubDetectPeopleFlag[0] + 1].uwYStart = Y_Pos;	
	tOSD_Img2(&tOsdImgInfo[2*tUI_CuSetting.ubDetectPeopleFlag[0] + 1], OSD_UPDATE);
