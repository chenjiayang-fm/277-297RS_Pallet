...
static void UI_CameraDrawSubSubSubMenuItem()
{
	...
	if(ubItemPreIdx != ubItemCurIdx)//只有当控件出现跳转才需要把前一个控件恢复为normal状态
	{
		...
		{
			//先擦除选中框
			tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_CAMS_BRIGHTNESS_GREEN_HL + 2*ubItemPreIdx, 1, &tOsdImgInfo[0]);
			...
			//slider
			tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_CAMS_BRIGHTNESS_SLIDER + ubItemPreIdx, 1, &tOsdImgInfo[0]);
			tOSD_Img2(&tOsdImgInfo[0],OSD_QUEUE);
			//round
	...
	{
		tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_CAMS_BRIGHTNESS_GREEN_HL + 2*ubItemCurIdx + CameraSubSubSubRemoteMode, 1, &tOsdImgInfo[0]);
		tOSD_Img2(&tOsdImgInfo[0],OSD_QUEUE);
		//word
		tOSD_GetOsdImgInfor(1, OSD_IMG2, ((tUI_CuSetting.tLanguage == LANGUAGE_ENGLISH)?OSD2IMG_CAMS_BRIGHTNESS_WORD:
										(tUI_CuSetting.tLanguage == LANGUAGE_GERMAN)?OSD2IMG_CAMS_BRIGHTNESS_WORD_GER:
		...
		tOSD_Img2(&tOsdImgInfo[0],OSD_QUEUE);
		//slider
		tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_CAMS_BRIGHTNESS_SLIDER + ubItemCurIdx, 1, &tOsdImgInfo[0]);
		tOSD_Img2(&tOsdImgInfo[0],OSD_QUEUE);
		//round
		tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_CAMS_ROUND_HL, 1, &tOsdImgInfo[1]);
...
void UI_AISubSubSubMenuExit()
{
	...
	//WORD
	tOSD_GetOsdImgInfor(1, OSD_IMG2, (tUI_CuSetting.tLanguage != LANGUAGE_GERMAN)?OSD2IMG_PARKINGLINE_TITLE:OSD2IMG_AI_TITLE, 1, &tOsdImgInfo[0]);
	if(tUI_CuSetting.tLanguage == LANGUAGE_FRENCH)
		tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_AI_TITLE, 1, &tOsdImgInfo[0]);
	else if(tUI_CuSetting.tLanguage == LANGUAGE_CHINESE)
		tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_AI_TITLE, 1, &tOsdImgInfo[0]);

	//WORD
	tOSD_GetOsdImgInfor(1, OSD_IMG2, (tUI_CuSetting.tLanguage != LANGUAGE_GERMAN)?OSD2IMG_PARKINGLINE_CAM1_WD:OSD2IMG_PARKINGLINE_CAM1_WD_GER, 4, &tOsdImgInfo[1]);
	...
	for(uint8_t i = 0;i < 4;i++)
		tOSD_Img2(&tOsdImgInfo[6*i + 2*tUI_CuSetting.ubIsEnableBSD[i]], OSD_QUEUE);	
	//high light
	tOSD_Img2(&tOsdImgInfo[2*tUI_CuSetting.ubIsEnableBSD[0] + 1], OSD_UPDATE);
	tUI_State = UI_SUBSUBMENU_STATE;
#else
	OSD_IMG_INFO tOsdImgInfo[24],tMenuOsdImgInfo;
	...
	//WORD
	tOSD_GetOsdImgInfor(1, OSD_IMG2, (tUI_CuSetting.tLanguage != LANGUAGE_GERMAN)?OSD2IMG_AI_TITLE:OSD2IMG_AI_TITLE_GEN, 1, &tOsdImgInfo[0]);
	if(tUI_CuSetting.tLanguage == LANGUAGE_FRENCH)
		tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_AI_TITLE_FR, 1, &tOsdImgInfo[0]);
	else if(tUI_CuSetting.tLanguage == LANGUAGE_CHINESE)
		tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_AI_TITLE_CHN, 1, &tOsdImgInfo[0]);
	tOSD_Img2(&tOsdImgInfo[0], OSD_QUEUE);

	tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_RETURN_NOR, 1, &tMenuOsdImgInfo);
	tOSD_Img2(&tMenuOsdImgInfo, OSD_QUEUE);
...
//				for(uint8_t i = 0;i < 4;i++)
//					{
//						tOsdImgInfo[2*tUI_CuSetting.ubIsEnableBSD[i]].uwXStart = X_Pos + X_Step*i;
//						tOsdImgInfo[2*tUI_CuSetting.ubIsEnableBSD[i]].uwYStart = Y_Pos + Y_Step*j;
//						tOSD_Img2(&tOsdImgInfo[2*tUI_CuSetting.ubIsEnableBSD[i]], OSD_QUEUE); 
//					}				
//				break;
			case 0:
				for(uint8_t i = 0;i < 4;i++)
					{
						tOsdImgInfo[2*tUI_CuSetting.ubDetectPeopleFlag[i]].uwXStart = X_Pos + X_Step*i;
						tOsdImgInfo[2*tUI_CuSetting.ubDetectPeopleFlag[i]].uwYStart = Y_Pos + Y_Step*j;
						tOSD_Img2(&tOsdImgInfo[2*tUI_CuSetting.ubDetectPeopleFlag[i]], OSD_QUEUE); 
					}				
				break;
			...
			case 1:
				for(uint8_t i = 0;i < 4;i++)
					{
						tOsdImgInfo[2*tUI_CuSetting.ubDetectCarFlag[i]].uwXStart = X_Pos + X_Step*i;
						tOsdImgInfo[2*tUI_CuSetting.ubDetectCarFlag[i]].uwYStart = Y_Pos + Y_Step*j;
						tOSD_Img2(&tOsdImgInfo[2*tUI_CuSetting.ubDetectCarFlag[i]], OSD_QUEUE); 
					}				
				break;
			...
			case 2:
				for(uint8_t i = 0;i < 4;i++)
					{
						tOsdImgInfo[2*tUI_CuSetting.ubIsEnableBSDALARM[i]].uwXStart = X_Pos + X_Step*i;
						tOsdImgInfo[2*tUI_CuSetting.ubIsEnableBSDALARM[i]].uwYStart = Y_Pos + Y_Step*j;
						tOSD_Img2(&tOsdImgInfo[2*tUI_CuSetting.ubIsEnableBSDALARM[i]], OSD_QUEUE); 
					}				
				break;
			...
			case 3:
				for(uint8_t i = 0;i < 4;i++)
					{
						tOsdImgInfo[2*tUI_CuSetting.ubBSDTriggerOut[i]].uwXStart = X_Pos + X_Step*i;
						tOsdImgInfo[2*tUI_CuSetting.ubBSDTriggerOut[i]].uwYStart = Y_Pos + Y_Step*j;
						tOSD_Img2(&tOsdImgInfo[2*tUI_CuSetting.ubBSDTriggerOut[i]], OSD_QUEUE); 
					}				
				break;
				...
				for(uint8_t i = 0;i < 4;i++)
					{
						...
						tOsdImgInfo[0].uwXStart = X_Pos + X_Step*i;
						tOsdImgInfo[0].uwYStart = Y_Pos + Y_Step*j;
						tOSD_Img2(&tOsdImgInfo[0], OSD_QUEUE); 
					}				
				break;
...
static void UI_TriggerSubSubSubMenuExit(uint16_t uwSubSubMenuItemIdx)
{
	...
	//button
	tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_TRIGGER_DELAY_CAM1_NOR, 20, &tOsdImgInfo_Button[0]);
...
static void UI_SettingingDrawSubSubSubMenuItem(uint16_t uwSubMenuItem,uint16_t uwSubSubMenuItemIdx)
{
	...
	if(uwSubMenuItem == SYSTEMSET_ITEM && uwSubSubMenuItemIdx == DATETIME_ITEM)
	{
		...
		if(ubItemPreIdx <= DATE_TIME_SECOND)
		{
			...
			tOSD_Img2(&tOsdImgInfo[2*ubItemPreIdx], OSD_QUEUE);
			if(uwTemp[ubItemPreIdx] < 10)
			{
				UI_ShowButtonValueNormal(0,tOsdImgInfo[2*ubItemPreIdx].uwXStart + 30,tOsdImgInfo[2*ubItemPreIdx].uwYStart + 15,OSD_QUEUE);
				UI_ShowButtonValueNormal(uwTemp[ubItemPreIdx],tOsdImgInfo[2*ubItemPreIdx].uwXStart + 50,tOsdImgInfo[2*ubItemPreIdx].uwYStart + 15,OSD_QUEUE);
			}
			else if(uwTemp[ubItemPreIdx] < 100)
				UI_ShowButtonValueNormal(uwTemp[ubItemPreIdx],tOsdImgInfo[2*ubItemPreIdx].uwXStart + 30,tOsdImgInfo[2*ubItemPreIdx].uwYStart + 15,OSD_QUEUE);
			else
				UI_ShowButtonValueNormal(tUI_CuSetting.tSysCalendar.uwYear,tOsdImgInfo[2*ubItemPreIdx].uwXStart + 10,tOsdImgInfo[2*ubItemPreIdx].uwYStart + 15,OSD_QUEUE);
		...
		{
			tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_DATE_TIME_YEAR_NOR, 12, &tOsdImgInfo[0]);
			tOSD_Img2(&tOsdImgInfo[2*ubItemIdx + 1], OSD_QUEUE);
			if(uwTemp[ubItemIdx] < 10)
			{
				UI_ShowButtonValueHighLight(0,tOsdImgInfo[2*ubItemIdx].uwXStart + 30,tOsdImgInfo[2*ubItemIdx].uwYStart + 15,OSD_UPDATE);
				UI_ShowButtonValueHighLight(uwTemp[ubItemIdx],tOsdImgInfo[2*ubItemIdx].uwXStart + 50,tOsdImgInfo[2*ubItemIdx].uwYStart + 15,OSD_UPDATE);
			}
			else if(uwTemp[ubItemIdx] < 100)
				UI_ShowButtonValueHighLight(uwTemp[ubItemIdx],tOsdImgInfo[2*ubItemIdx].uwXStart + 30,tOsdImgInfo[2*ubItemIdx].uwYStart + 15,OSD_UPDATE);
			else
				UI_ShowButtonValueHighLight(tUI_CuSetting.tSysCalendar.uwYear,tOsdImgInfo[2*ubItemIdx].uwXStart + 10,tOsdImgInfo[2*ubItemIdx].uwYStart + 15,OSD_UPDATE);
	...
	switch(Touch_Info->Gesture)
	{
		...
		//滑动条
			if(tUI_CamStatus[uwSubSubMenuItemIdx].tCamConnSts == CAM_ONLINE)
			{
				...
				for(i = 0;i < 5;i++)
				{
					tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_CAMS_BRIGHTNESS_SLIDER + i,1,&tOsdImgInfo);
