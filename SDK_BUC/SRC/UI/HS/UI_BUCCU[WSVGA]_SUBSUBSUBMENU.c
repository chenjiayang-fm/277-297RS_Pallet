...
void UI_PlaybackSubSubSubMenuPage(UI_ArrowKey_t tArrowKey)
{
	...
	switch(tArrowKey)
	{
		...
		case RIGHT_ARROW:
			...
			break;
		case ENTER_ARROW:
			if(UI_RECFILE_PLAY != tUI_RecPlayAct.tPlaySts)//µ±«∞“—Õ£÷π≤•∑≈
			{
			...
			{
				if(UI_RECPLAYPAUSE_ITEM == tUI_RecPlayListItem.ubItemIdx)//≤•∑≈Õ£÷π∞¥≈•
				{
				...
				if(UI_RECFILE_PAUSE != tUI_RecPlaySts)//÷ª”–‘⁄≤•∑≈◊¥Ã¨≤≈ø…“‘Ω¯––Ω¯∂»µ˜Ω⁄∫Õ“Ù‘¥—°‘Ò
				{
					...
					else if(UI_RECADOSRCSEL_ITEM == tUI_RecPlayListItem.ubItemIdx)
					{
						if(tUI_RecPlayAct.ubPlayMode == UI_RECPLAY_SINGLEVIEW)
							return;
						if(ubPLY_Pause(PLY_PAUSE_ON) == 0)//Õ£÷π≤•∑≈£¨—°ÕÍ“Ù∆µ∫Û‘ŸºÃ–¯≤•∑≈
	                    	return;
...
static void UI_CameraDrawSubSubSubMenuItem()
{
	...
	//-----------------------------------------preIdx------------------------------------------
	if(ubItemPreIdx != ubItemCurIdx)//÷ª”–µ±øÿº˛≥ˆœ÷Ã¯◊™≤≈–Ë“™∞—«∞“ª∏ˆøÿº˛ª÷∏¥Œ™normal◊¥Ã¨
	{
...
void UI_CameraSubSubSubMenuPage(UI_ArrowKey_t tArrowKey)
{
	...
	{
		//µ±«∞Ω¯∂»Ãıµƒ∏ﬂ¡¡—’…´Œ™¬Ã…´£¨“£øÿµƒ◊Û”“º¸ ««–ªª≤ªÕ¨µƒ—°œÓ
		if(CameraSubSubSubRemoteMode == REMOTE_SELECT_ITEM)
		{
			...
			UI_CameraDrawSubSubSubMenuItem();
		}//Ω¯∂»Ãıµƒ∏ﬂ¡¡±‰Œ™∫Ï…´£¨◊Û”“º¸ø…“‘µ˜Ω⁄Ω¯∂»Ãıµƒ ˝÷µ
		else
		{	
			uint8_t ubItemCurIdx = tCameraSubSubSubMenuItem[uwSubSubMenuItemIdx]->tSubMenuInfo.ubItemIdx;
			switch(tArrowKey)
				...
				else if(ubSubSubMenuItem == LANGUAGE_ITEM)
				{
					if(tUI_CuSetting.tLanguage != (UI_Language_t)tSystemSubSubSubMenuItem[ubSubSubMenuItem].tSubMenuInfo.ubItemIdx)
					{
						tUI_CuSetting.tLanguage = (UI_Language_t)tSystemSubSubSubMenuItem[ubSubSubMenuItem].tSubMenuInfo.ubItemIdx;
							//∏˘æ›”Ô—‘–ﬁ∏ƒ ±«¯
						if(tUI_CuSetting.tLanguage == LANGUAGE_ENGLISH)//Œ˜¡˘
							tUI_CuSetting.ubtimezone = 6;
						else if(tUI_CuSetting.tLanguage == LANGUAGE_GERMAN)//∂´“ª
							tUI_CuSetting.ubtimezone = 14;
						else if(tUI_CuSetting.tLanguage == LANGUAGE_FRENCH)//∂´“ª
							tUI_CuSetting.ubtimezone = 14;
						else if(tUI_CuSetting.tLanguage == LANGUAGE_CHINESE)//∂´∞À
...
//	OSD_EraserImg2(&tOsdImgInfo);
	...
	UI_DrawParkingLine(CamIdx,OSD_QUEUE);	//œ‘ æ∑µªÿÕº±Í
	if(ubUI_TouchPanelSts)
	{
...
void UI_ParkingLineSubSubSubMenuPage(UI_ArrowKey_t tArrowKey)
{
	...
	uint16_t uwTempVal;
	uint16_t uwHSize = 0,uwVSize = 0; 
	//Êì¶Èô§‰πãÂâçÁöÑÂÄíËΩ¶ÂÖâÊ†á
	UI_EraseParkingLine(CamIdx,OSD_QUEUE);
	
	//Ëé∑ÂèñÂÄíËΩ¶ÂÖâÊ†áÁöÑÂ§ßÂ∞?
	tOSD_GetOsdImgInfor(1, OSD_IMG1, OSD1IMG_PARKINGLINE_LEFT, 1, &tParkingLineOsdImgInfo);
	uwHSize += 2*tParkingLineOsdImgInfo.uwHSize;
	uwVSize = tParkingLineOsdImgInfo.uwVSize;
	tOSD_GetOsdImgInfor(1, OSD_IMG1, OSD1IMG_PARKINGLINE_UPPER1 + tUI_CuSetting.ParkingLineSize[CamIdx], 1, &tParkingLineOsdImgInfo);
...
void UI_GuideLineSubSubSubMenuPageExit(void)
{
	...
	// Ê†àÊùø‰ΩøÁî®ÂΩìÂâçÈÄöÈÅìÂºïÂØºÁ∫øÔºåÊ†áÂÆöÈÄÄÂá∫Êó∂ËøûÂêåÂ∑≤Á°ÆËÆ§ÁöÑÈÄöÈÅìÈÄâÊã©ÂêåÊ≠•„ÄÇ
...
void UI_GuideLineSubSubSubMenuPage(UI_ArrowKey_t tArrowKey)
{
	...
	uint16_t uwTempVal;
	tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_GUIDELINE_SIZE1 + tUI_CuSetting.GuideLineSize[CamIdx], 1, &tOsdImgInfo[0]);
	//ÔøΩÔøΩÔøΩÔøΩ÷Æ«∞ÔøΩƒµÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩÔøΩ“£ÔøΩÿ≤ÔøΩÔøΩÔøΩ◊¥Ã¨
...
//    }
//};
//uint8_t Location1[4] = {4,4,4,4}; // Á¨¨‰∫åÊù°Á∫øÂú®Êï¥‰∏™Ê°Ü‰∏≠ÁöÑÊØî‰æã
//uint8_t Location2[4] = {8,8,8,8}; // Á¨¨‰∏âÊù°Á∫øÂú®Êï¥‰∏™Ê°Ü‰∏≠ÁöÑÊØî‰æã
...
//		{
//			tOSD_GetOsdImgInfor(1, OSD_IMG2,OSD2IMG_AI_FRAME_TYPE1_NOR , 4, &tOsdImgInfo[0]);//Á±ªÂûã‰∏ÄÂà§Êñ≠
//			if(tUI_CuSetting.ubBSDBoxType)//ÂÄº‰∏∫1 Á±ªÂûã‰∫?
//				tOSD_Img2(&tOsdImgInfo[0], OSD_QUEUE);//Ê≤°ÈÄâ‰∏≠ÂÜÖÈÉ®Â≠ó‰Ωì‰∏∫ÁôΩËâ?
//			else
//				tOSD_Img2(&tOsdImgInfo[2], OSD_QUEUE);//ÈÄâ‰∏≠ÂÜÖÈÉ®Â≠ó‰Ωì‰∏∫ÁªøËâ?
//		}
//		else if(ubItemPreIdx == BSDRANGE_TYPE2_ITEM)
//		{
//			tOSD_GetOsdImgInfor(1, OSD_IMG2,OSD2IMG_AI_FRAME_TYPE2_NOR , 4, &tOsdImgInfo[0]);//Á±ªÂûã‰∫åÂà§Êñ?
//			if(tUI_CuSetting.ubBSDBoxType)//ÂÄº‰∏∫1 Á±ªÂûã‰∫?
//				tOSD_Img2(&tOsdImgInfo[2], OSD_QUEUE);//ÈÄâ‰∏≠ÂÜÖÈÉ®Â≠ó‰Ωì‰∏∫ÁªøËâ?
//			else
//				tOSD_Img2(&tOsdImgInfo[0], OSD_QUEUE);//Ê≤°ÈÄâ‰∏≠ÂÜÖÈÉ®Â≠ó‰Ωì‰∏∫ÁôΩËâ?
...
//				tOSD_Img2(&tOsdImgInfo[1], OSD_UPDATE);//ÈÄâ‰∏≠ÂÜÖÈÉ®Â≠ó‰Ωì‰∏∫ÁªøËâ?
//			else
//				tOSD_Img2(&tOsdImgInfo[3], OSD_UPDATE);//Ê≤°ÈÄâ‰∏≠ÂÜÖÈÉ®Â≠ó‰Ωì‰∏∫ÁôΩËâ?
////		}
////		else
////		{	
////			if(tUI_CuSetting.ubBSDBoxType)
////				tOSD_Img2(&tOsdImgInfo[0], OSD_UPDATE);//ÈÄâ‰∏≠ÂÜÖÈÉ®Â≠ó‰Ωì‰∏∫ÁªøËâ?
////			else
////				tOSD_Img2(&tOsdImgInfo[2], OSD_UPDATE);//Ê≤°ÈÄâ‰∏≠ÂÜÖÈÉ®Â≠ó‰Ωì‰∏∫ÁôΩËâ?
////		}
//	}
...
void UI_AISubSubSubMenuPage(UI_ArrowKey_t tArrowKey)
{
	...
	uint8_t CamIdx = tSettingSubSubMenuItem.tSettingS[10].tSubMenuInfo.ubItemIdx % 4;//ËÆ°ÁÆóÁé∞Âú®ÊòØÁ¨¨Âá†‰∏™ÈÄöÈÅì
...
//					//Â§çÁî®Âà∑ÂõæÁöÑÂáΩÊï∞ÔºåÂà∑Âõæ
...
//√ø¥ŒΩ¯»Îœ‡”¶subsubsub≤Àµ•–Ë“™∏¸–¬œ‡”¶ubItemIdxµƒ≥ı º÷µ
	...
	if(uwSubSubMenuItemIdx == SDCARD_ITEM)
	{
		...
		for(uint8_t i = 0;i < 3;i++)
		{
			if(tUI_CuSetting.RecInfo.tREC_Time == i)
				tOSD_Img2(&tOsdImgInfo[2*i + 6], OSD_QUEUE);//∏ﬂ¡¡Õº±Í…œµƒŒƒ◊÷
			else
				tOSD_Img2(&tOsdImgInfo[2*i], OSD_QUEUE);
		}
...
void UI_PlaybackSubSubSubTouchMenu(TOUCH_EVENT_t *Touch_Info)
{
	...
	switch(Touch_Info->Gesture)
	{
		case TOUCH_PRESS:
			...
			//Ω‚Œˆ ÷÷∏µ„ª˜‘⁄ƒƒ∏ˆÕº±Í…œ
			for(i = 0; i < UI_RECPLAYLISTITEM_MAX;i++)//≈–∂œæﬂÃÂµ„ª˜‘⁄ƒƒ∏ˆÕº±Í…œ
			{
				//∂¡»°Õº∆¨–≈œ¢
			...
			if(UI_RECPLAYLISTITEM_MAX == i)//√ª”–∞¥µΩ”––ßµƒøÿº˛
				return;
...
{
	OSD_IMG_INFO tOsdImgInfo;
	static uint8_t ubTouchPressDownCount = 0;//∏√±‰¡ø «∏¯TOUCH_PRESSDOWNΩµÀŸ”√µƒ
	uint8_t i;
	uint16_t uwSubSubMenuItemIdx = 
		(uint16_t)tSettingSubSubMenuItem.tSettingS[CAMERASET_ITEM].tSubMenuInfo.ubItemIdx;
	...
	switch(Touch_Info->Gesture)
	{
		case TOUCH_PRESS:
			...
			if(Touch_Info->startY < tOsdImgInfo.uwYStart + tOsdImgInfo.uwVSize)//sub subΩÁ√Ê
			{
				for(i = 0;i < 4;i++)
				{
					//∂¡»°Õº∆¨–≈œ¢
		...
		case TOUCH_PRESS:
			if (ubSubSubMenuItem == UPGRADE_ITEM || ubSubSubMenuItem == LANGUAGE_ITEM || ubSubSubMenuItem == DATETIME_ITEM)
			{
				for (i = 0; i < ubMenuItemCount; i ++)
				{
                    if (ubSubSubMenuItem == DATETIME_ITEM)
                    {
						if(i > DATE_TIME_SECOND)//Ë∑≥ËøáÊªëÂä®Êù?
						{						
...
void UI_ParkingLIneSubSubSubTouchMenu(TOUCH_EVENT_t *Touch_Info)
{
	...
	switch(Touch_Info->Gesture)
	{
		...
		case TOUCH_TURNDOWN:
			break;
		case TOUCH_PRESSDOWN:
			//»Áπ˚µ„ª˜‘⁄∑µªÿÕº±Í…œæÕ≤ª÷¥––pressDown∂Ø◊˜
			tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_RETURN_NOBACKGROUND_HL, 1, &tOsdImgInfo);	
			if(Touch_Info->startX > tOsdImgInfo.uwXStart 
				&& Touch_Info->startX < tOsdImgInfo.uwXStart + tOsdImgInfo.uwHSize
				&& Touch_Info->startY > tOsdImgInfo.uwYStart 
...
void UI_AISubSubSubTouchMenu(TOUCH_EVENT_t *Touch_Info)
{
	...
	{
		case TOUCH_PRESS:				
			for(i = 0; i < AIITEM_MAX;i++)//≈–∂œæﬂÃÂµ„ª˜‘⁄ƒƒ∏ˆÕº±Í…œ
			{	

				tOSD_GetOsdImgInfor(1, OSD_IMG2, uwAISubSubSubMenuItemOsdImg[i], 1, &tOsdImgInfo);	
				if(i == BSDRANGE_ENABLE_ITEM)//BSDRANGEÂºÄÂÖ≥ÂõæÊ†áÂ§çÁî®H MirrorÔºåÈúÄË¶ÅÂçïÁã¨Â§ÑÁêÜ‰ΩçÁΩ?
				{	
...
//settingµƒ¥Û≤ø∑÷ΩÁ√Êµƒ≤Ÿ◊˜¬ﬂº≠∂º≤Ó≤ª∂‡£¨À˘“‘√ª”–Ãÿ ‚–Ë«ÛµƒΩÁ√Ê∂º π”√Õ¨“ªÃ◊¥•√˛Ω‚Œˆ¥˙¬Î
//------------------------------------------------------------------------------------universal------------------------------------------------------------------------------
...
//uwFirstImageIndexOfSubSubSub[][]¥Êµƒ «subsubsubΩÁ√Êµƒµ⁄“ª’≈Õº∆¨
	...
	switch(Touch_Info->Gesture)
	{
		case TOUCH_PRESS:
			if(uwFirstImageIndexOfSubSubSubSetting[tSubMenuItem][uwSubSubMenuItemIdx] == 0)
				return;
			for(i = 0; i < ubSubSubSubMenuItemCount;i++)//≈–∂œæﬂÃÂµ„ª˜‘⁄ƒƒ∏ˆÕº±Í…œ
			{
				//∂¡»°Õº∆¨–≈œ¢
				if(tSubMenuItem == RECORDSET_ITEM && uwSubSubMenuItemIdx == DATETIME_ITEM && i > DATE_TIME_SECOND)//Ã¯π˝ª¨∂ØÃı
				{						
					i = DATE_TIME_UTC_COMFIRM;
					tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_DATE_TIME_UTC_COMFIRM_NOR,1, &tOsdImgInfo);
				}
