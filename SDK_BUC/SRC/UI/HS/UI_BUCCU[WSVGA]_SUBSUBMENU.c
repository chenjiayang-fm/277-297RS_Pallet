...
void UI_PlaybackSubSubMenuPage(UI_ArrowKey_t tArrowKey)
{
	...
	//ªª“≥
	if((((!(tUI_RecFilesInfo.uwRecFileSelIdx % REC_FILE_LIST_MAXNUM)) && (tUI_RecFilesInfo.uwRecFileSelIdx) && (ubUI_PrevRecFileIdx == REC_FILE_LIST_MAXNUM - 1))) ||//œ¬“ª“≥
	   ((ubUI_PrevRecFileIdx == 0) && (ubUI_RecFileIdx == REC_FILE_LIST_MAXNUM - 1)))//…œ“ª“≥
	{
	...
	else if(ubItemIdx == LANGUAGE_ITEM)
	{
		...
		for(uint8_t i = 0;i < LANGUAGE_MAX;i ++)
		{
			if(tUI_CuSetting.tLanguage == i)
				tOSD_Img2(&tOsdImgInfo[2*i + 2*LANGUAGE_MAX], OSD_QUEUE);//∏ﬂ¡¡Õº±Í…œµƒŒƒ◊÷
			else
...
// ÂºπÁ™óÊúüÈó¥‰ªÖËÆ∞ÂΩïÂæÖÈÄâÈ°πÔºåÈÄâÂê¶/ËøîÂõû‰∏ç‰øÆÊîπ‰ªª‰ΩïÈÄöÈÅìÈÖçÁΩÆ„ÄÇ
static uint8_t ubAIConfirm = FALSE;
static uint8_t ubAIConfirmItem = 1; // ÈªòËÆ§ÈÄâÂê¶„ÄÇ
...
// ËåÉÂõ¥Êï∞ÁªÑ‰∏é Pallet È°µÈù¢ÁöÑÂÖ≠Ë°åÂèÇÊï∞È°∫Â∫è‰∏ÄËá¥ÔºåÁªòÂà∂„ÄÅÊåâÈîÆÂíåËß¶Êë∏ÂÖ±Áî®„ÄÇ
...
void UI_AIDrawSubSubMenuItem(uint8_t ubDrawBox)
{
	...
	// ‰∏ªÈ°µÈù¢Á¥¢ÂºïÊåâ BSD„ÄÅPallet„ÄÅSETUP ‰∏âÂàóÂàÜÁªÑÔºåÊØèÂàóÂåÖÂê´Âõõ‰∏™ÈÄöÈÅì„ÄÇ
	...
	// ÊªëÊù°ÊåâÈ°µÈù¢È°∫Â∫èÊéíÂàóÔºåRate Range Âú® Flow Frame Interval ‰πãÂâç„ÄÇ
	...
	{
		// Âõõ‰∏™ÂºÄÂÖ≥Êåâ‰∏§ÂàóÊéíÂàóÔºåÊúÄÂêé‰∏ÄÈ°π‰∏∫ Cursor SETUP„ÄÇ
	...
	else
	{
		...
		{
			// Â§çÁî® 486x56 ÁöÑ‰∏çÈÄèÊòéÊ°ÜÔºåÂè™Ê†áËÆ∞Âè≥‰æßÊªëÊù°ÔºåÊñáÂ≠ó‰∏çËøõÂÖ•ÈÄâ‰∏≠Ê°Ü„ÄÇ
			...
			// ÂÖ≠Ë°åÊñáÂ≠óÂíåÊªëÊù°‰ΩøÁî®Áõ∏ÂêåË°åË∑ùÔºåÊ®™ÂùêÊ†áÁõ¥Êé•‰ΩøÁî®ÂØπÂ∫îËØ≠Ë®ÄÂõæÁâáÁöÑÈÖçÁΩÆ„ÄÇ
			...
			// ÊªëÂùó‰ΩøÁî® System ÁöÑ‰∏çÈÄèÊòéËµÑÊ∫êÔºåÂêÑË°åÊ†πÊçÆËá™Â∑±ÁöÑËåÉÂõ¥ÂÆö‰Ωç„ÄÇ
...
static void UI_AIConfirmExecute(void)
{
	...
	{
		// Á°ÆËÆ§ÂêéÂÜçÊääÂéüÊ†àÊùøÈÄöÈÅìÊîπ‰∏∫ BSDÔºåÂõõË∑ØÈÄâÊã©ÂêàÂπ∂ÂèëÈÄÅ„ÄÇ
...
static void UI_AISetupExecute(void)
{
	...
	if(ubAISetupPage == AI_PAGE_ALGORITHM)
	{
		// BSD/Pallet ÊåâÈíÆÂè™ÂàáÊç¢ÁÆóÊ≥ïÔºåSETUP ÊåâÈíÆÊâìÂºÄÂΩìÂâçÈÄöÈÅìÁöÑËÆæÁΩÆÈ°µ„ÄÇ
	...
	else if(ubAISetupPage == AI_PAGE_BSD)
	{
		switch(ubItemIdx)
		{
			...
			case 4:
				// Cursor SETUP ËøõÂÖ•ÂΩìÂâçÈÄöÈÅìÁöÑÊ£ÄÊµãÂå∫ÂüüÁºñËæëÈ°µ„ÄÇ
	...
	{
		// ÂÖ≠‰∏™ÊªëÊù°Áªü‰∏Ä‰ΩøÁî®Á°ÆËÆ§ÈîÆÂàáÊç¢ÈÄâÊã©/Ë∞ÉÊï¥Ê®°ÂºèÔºåË∞ÉÊï¥Êó∂ÂÖÅËÆ∏Â∑¶Âè≥ÈîÆËøûÂèë„ÄÇ
...
void UI_AISubSubMenuPage(UI_ArrowKey_t tArrowKey)
{
	...
	// AI ‰∏ªÈ°µÈù¢‰∏éÁÆóÊ≥ïËÆæÁΩÆÈ°µÂàÜÂà´‰ΩøÁî®ÂêÑËá™ÁöÑËèúÂçïÁÑ¶ÁÇπ„ÄÇ
	...
	if(tArrowKey == EXIT_ARROW)
	{
		// ËÆæÁΩÆÈ°µÈÄÄÂá∫Êó∂‰øùÂ≠òÂπ∂ËøîÂõû AI ‰∏ªÈ°µÈù¢Ôºå‰∏ªÈ°µÈù¢ÈÄÄÂá∫Êó∂ËøîÂõû‰∏äÁ∫ßËèúÂçï„ÄÇ
		SendIrCodeFlag = FALSE;
		ubPalletRemoteMode = REMOTE_SELECT_ITEM;
		if(ubAISetupPage != AI_PAGE_ALGORITHM)
		{
			...
			{
				// ÈÄâÊã©Êú™ÂèòÔºå1126 Âè™Êõ¥Êñ∞ÂèÇÊï∞Âπ∂Â∫îÁ≠îÔºå‰∏çÈáçÂêØÔºõ‰∏¢Â∏ßÂèØÊåâÂéüÈÖçÁΩÆÈáçÂèë„ÄÇ
				ubAIConfigSync = TRUE;
				UI_SendAIConfigTo1126();
			}
			UI_AIEnterMenu();
	...
	// Ë∞ÉÊï¥Ê®°Âºè‰∏ãÊåâÂΩìÂâçË°åÁöÑÊ≠•ÈïøÂä†ÂáèÔºåÂà∞Ëææ‰∏ä‰∏ãÈôêÂÅúÊ≠¢ÔºåÁ°ÆËÆ§ÈîÆÁªìÊùüË∞ÉÊï¥„ÄÇ
	...
	// ÊôÆÈÄöÊ®°Âºè‰∏ãÔºåÂ∑¶Âè≥ÈîÆÁßªÂä®ÁÑ¶ÁÇπÔºåÁ°ÆËÆ§ÈîÆÊâßË°åÂΩìÂâçÈ°π„ÄÇ
...
void UI_DualDrawSubSubMenuItem()
{
	...
	if(ubItemPreIdx != ubItemCurIdx)
	{
		...
		{
			if(tUI_CamStatus[ubItemPreIdx].tCamDispLocation_Dual == DISP_LEFT)//Œƒ◊÷∏ﬂ¡¡
		...
		{
			ubItemPreIdx -= 4;
			if(tUI_CamStatus[ubItemPreIdx].tCamDispLocation_Dual == DISP_RIGHT)//Œƒ◊÷∏ﬂ¡¡
			{
	...
	{
		if(tUI_CamStatus[ubItemCurIdx].tCamDispLocation_Dual == DISP_LEFT)//Œƒ◊÷∏ﬂ¡¡
	...
	{
		ubItemCurIdx -= 4;
		if(tUI_CamStatus[ubItemCurIdx].tCamDispLocation_Dual == DISP_RIGHT)//Œƒ◊÷∏ﬂ¡¡
		{
...
void UI_DualSubSubMenuExecute()
{
	...
	{
		/*ΩªªªŒª÷√
		* ’“µΩŒª÷√Œ™DISP_LEFTµƒtx£¨∞—∏√txµƒŒª÷√∏ƒŒ™µ±«∞—°÷–µƒtxµƒŒª÷√
		* »ª∫Ûµ±«∞—°÷–µƒtxµƒŒª÷√±‰Œ™DISP_LEFT
		*/
		for(uint8_t i = 0;i < 4;i++)
		{
...
void UI_QuadDrawSubSubMenuItem()
{
	...
	for(uint8_t i = 0;i < 2;i++)
	{
		...
		{
			if(tUI_CamStatus[ubItemIdx[i]].tCamDispLocation_Quad == DISP_UPPER_LEFT)//Œƒ◊÷∏ﬂ¡¡
			{
		...
		{
			ubItemIdx[i] -= 4;
			if(tUI_CamStatus[ubItemIdx[i]].tCamDispLocation_Quad == DISP_UPPER_RIGHT)//Œƒ◊÷∏ﬂ¡¡
			{
		...
		{
			ubItemIdx[i] -= 8;
			if(tUI_CamStatus[ubItemIdx[i]].tCamDispLocation_Quad == DISP_LOWER_LEFT)//Œƒ◊÷∏ﬂ¡¡
			{
				tCamOsdImgInfo[8 + 2*ubItemIdx[i] + i].uwYStart += uwLcd_VSize/4;
				tOSD_Img2(&tCamOsdImgInfo[8 + 2*ubItemIdx[i] + i], (i == 1)?OSD_UPDATE:OSD_QUEUE);
			}
		...
		{
			ubItemIdx[i] -= 12;
			if(tUI_CamStatus[ubItemIdx[i]].tCamDispLocation_Quad == DISP_LOWER_RIGHT)//Œƒ◊÷∏ﬂ¡¡
			{
				tCamOsdImgInfo[8 + 2*ubItemIdx[i] + i].uwXStart += uwLcd_HSize/2;
				tCamOsdImgInfo[8 + 2*ubItemIdx[i] + i].uwYStart += uwLcd_VSize/4;
				tOSD_Img2(&tCamOsdImgInfo[8 + 2*ubItemIdx[i] + i], (i == 1)?OSD_UPDATE:OSD_QUEUE);
...
//settingµƒ¥Û≤ø∑÷ΩÁ√Êµƒ≤Ÿ◊˜¬ﬂº≠∂º≤Ó≤ª∂‡£¨À˘“‘√ª”–Ãÿ ‚–Ë«ÛµƒΩÁ√Ê∂º π”√Õ¨“ªÃ◊“£øÿΩ‚Œˆ¥˙¬Î
//------------------------------------------------------------------------------------universal------------------------------------------------------------------------------
...
{	
	...
	for(uint8_t i = 0;i < 4;i++)
		tOSD_Img2(&tOsdImgInfo[2*i], OSD_QUEUE);//œ»∞—À˘“‘camŒƒ◊÷∂ºÀ¢Œ™normal◊¥Ã¨
...
static void UI_AutoScanSubSubMenuExecute(uint16_t uwSubSubMenuItemIdx)
{
	...
	if(uwSubSubMenuItemIdx%2 == 0)
	{
		...
		if(ubCount == 0)//◊‘∂Ø…®√Ë÷¡…Ÿ–Ë“™“ª∏ˆª≠√Ê≤Œ”Î
		{
			tUI_CuSetting.AutoScanEnable[CamIdx] = 1 - tUI_CuSetting.AutoScanEnable[CamIdx];
			return;
		}
...
void UI_AISubSubMenuExecute(uint8_t ubItemCurIdx,uint8_t Switch_FLAG)
{
	...
	if(Switch_FLAG)
	{
		//Èò≤Ê≠¢Ëá™Âä®ÈÄÄÂá∫ËèúÂçïËÄåÁ¥¢ÂºïÊú™Ê∏?
...
//		tOSD_Img2(&tOsdImgInfo[0], OSD_QUEUE);
		//AI CONFIG 
//		//ÂêéÈù¢ÈúÄË¶ÅÂä†Âà§Êñ≠ÔºåÈÄâÊã©‰∫Ü‰ªÄ‰πàÁ±ªÂûãÁöÑÁîªÊ°Ü
//		if(tUI_CuSetting.ubBSDBoxType)//ÈÄâÊã©‰∫ÜÁ±ªÂûã‰∫å 1
//		{
...
//		}
		...
		// BSD ÁîªÊ°ÜÂáΩÊï∞APIÈ¢ÑÁïô
		//UI_AutoDrawBSD(ubItemCurIdx%4); 
		//UI_AIDrawPoint(AILinePointIndex,CamIdx);
		tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_CAMS_H_MIRROR_OFF_NOR, 4, &tOsdImgInfo[0]);//Â§çÁî®H mirrorÁöÑON/OFFÂõ?
...
//			}
	...
	tUI_State = UI_SUBSUBSUBMENU_STATE;
	SendIrCodeFlag = 1;//ËøõÂÖ•Â≠êÁ∫ßËèúÂçï
		...
		case SETTING_ITEM:
			{
				...
				UI_SettingSubMenuItemList_t tSubMenuItem = 
						(UI_SettingSubMenuItemList_t)tUI_SubMenuItem[SETTING_ITEM].tSubMenuInfo.ubItemIdx;
				switch(tSubMenuItem)
				{	//“‘œ¬ΩÁ√ÊŒﬁ∑®”√∫Ø ˝UI_SettingSubSubTouchMenuPage¿¥Ω¯––¥•√˛Ω‚Œˆ£¨À˘“‘÷ÿ–¬”√“ª∏ˆ∫Ø ˝
					case RECORDSET_ITEM:
...
void UI_PlaybackSubSubTouchMenuPage(TOUCH_EVENT_t *Touch_Info)
{
	...
	switch(Touch_Info->Gesture)
	{
		case TOUCH_PRESS:	
			...
			if(i < REC_FILE_LIST_MAXNUM)//µ„ª˜‘⁄Œƒº˛¡–±Ì«¯”Ú
			{
				if((tUI_RecFilesInfo.uwRecFileSelIdx + i - ubUI_RecFileIdx) >= tUI_RecFilesInfo.uwTotalRecFileNum)//µ±«∞µ„ª˜µƒŒª÷√√ª”–Œƒº˛
					return;
...
void UI_AISubSubTouchMenuPage(TOUCH_EVENT_t *Touch_Info)
{
	...
	if(ubAIConfirm)
	{
		...
		return; // ÂºπÁ™óÂ§ñËß¶Êë∏‰∏çÁ©øÈÄèÂà∞ÁÆóÊ≥ïÊåâÈíÆ„ÄÇ
	}
	if(Touch_Info->Gesture == TOUCH_PRESS)
	{
		// ËøîÂõûÊåâÈíÆÂÖàÊòæÁ§∫È´ò‰∫ÆÔºåÂÜçÂ§çÁî®ÊåâÈîÆÈÄÄÂá∫ÈÄªËæëÂÆåÊàê‰øùÂ≠òÂíåËøîÂõû„ÄÇ
		tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_RETURN_HL, 1, &tOsdImgInfo);
		if(Touch_Info->startX >= tOsdImgInfo.uwXStart && Touch_Info->startX < tOsdImgInfo.uwXStart + tOsdImgInfo.uwHSize
			&& Touch_Info->startY >= tOsdImgInfo.uwYStart && Touch_Info->startY < tOsdImgInfo.uwYStart + tOsdImgInfo.uwVSize)
		{
		...
		if(ubAISetupPage == AI_PAGE_ALGORITHM)
		{
			...
			for(i = 0;i < CAM_4T;i++)
				for(j = 0;j < AI_MAIN_COLUMN_COUNT;j++)
				{
					...
					{
						// Ê†πÊçÆËß¶Êë∏ÂëΩ‰∏≠ÁöÑË°å„ÄÅÂàóËøòÂéü‰∏ªÈ°µÈù¢ËèúÂçïÁ¥¢Âºï„ÄÇ
		...
		{
			tOSD_GetOsdImgInfor(1, OSD_IMG2, OSD2IMG_AI_CONFIG_SETUP_NOR, 1, &tOsdImgInfo);
			// ‰ΩøÁî®‰∏éÁªòÂà∂Áõ∏ÂêåÁöÑ‰∏§ÂàóÂ∏ÉÂ±ÄÔºåÂà§Êñ≠ÂºÄÂÖ≥Âíå Cursor SETUP ÁöÑËß¶Êë∏ÂëΩ‰∏≠„ÄÇ
			for(i = 0;i < AI_BSD_ITEM_COUNT;i++)
			{
	...
	// ÂÖ≠Ë°åÊªëÊù°ÂùáÊîØÊåÅÁÇπÂáªÂíåÊãñÂä®ÔºåÊãñÂä®Êó∂‰ΩøÁî®ÊúÄÊñ∞Ëß¶ÁÇπÂùêÊ†á„ÄÇ
	if(ubAISetupPage == AI_PAGE_PALLET && (Touch_Info->Gesture == TOUCH_PRESS || Touch_Info->Gesture == TOUCH_PRESSDOWN))
	{
		...
		for(i = 0;i < AI_PALLET_PARAM_COUNT;i++)
		{
			...
			{
				// ÊåâÊªëÂùó‰∏≠ÂøÉÁöÑ‰ΩçÁΩÆÊç¢ÁÆóÊï∞ÂÄºÔºåÂπ∂Âê∏ÈôÑÂà∞ÂΩìÂâçÂèÇÊï∞ÂÖÅËÆ∏ÁöÑÊ≠•Èïø„ÄÇ
...
void UI_DualSubSubTouchMenuPage(TOUCH_EVENT_t *Touch_Info)
{
	...
	{
		case TOUCH_PRESS:	
			for(i = 0; i < DUAL_MAX;i++)//≈–∂œæﬂÃÂµ„ª˜‘⁄ƒƒ∏ˆÕº±Í…œ
			{	
...
void UI_QuadSubSubTouchMenuPage(TOUCH_EVENT_t *Touch_Info)
{
	...
	{
		case TOUCH_PRESS:	
			for(i = 0; i < QUAD_MAX;i++)//≈–∂œæﬂÃÂµ„ª˜‘⁄ƒƒ∏ˆÕº±Í…œ
			{	
...
void UI_AutoScanSubSubTouchMenuPage(TOUCH_EVENT_t *Touch_Info)
{
	...
	{
		case TOUCH_PRESS:	
			for(i = 0; i < tSettingSubSubMenuItem.tSettingS[uwSubMenuItem].ubItemCount;i++)//≈–∂œæﬂÃÂµ„ª˜‘⁄ƒƒ∏ˆÕº±Í…œ
			{	
				//∂¡»°µ±«∞≤Ÿ◊˜ΩÁ√Êµƒ»´≤øÕº±Í–≈œ¢
...
//------------------------------------------------------------------------------parkingLine-----------------------------------------------------------------------
//∏√ΩÁ√ÊŒﬁ∑® π”√∫Ø ˝UI_SettingSubSubTouchMenuPageΩ¯––¥•√˛ ∂±£¨À˘“‘µ•∂¿ π”√“ª∏ˆ∫Ø ˝Ω¯––Ω‚Œˆ
void UI_ParkingLineSubSubTouchMenuPage(TOUCH_EVENT_t *Touch_Info)
{
...
//∑«Ãÿ ‚ΩÁ√Ê∂ºø…“‘‘⁄œ¬√Ê∫Ø ˝÷–Ω¯––¥•√˛Ω‚Œˆ
//------------------------------------------------------------------------------universal-----------------------------------------------------------------------

//∏√ ˝◊È¥Êµƒ «√ø∏ˆΩÁ√Êµƒ∆ ºÕº∆¨–Ú∫≈
