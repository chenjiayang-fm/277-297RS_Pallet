		...
		if(ulGKEY_DetEnFlag & (1 << i))
		{
            if(!(ulGKEY_Polar & (1 << i)))
            {
                if((ulGKEY_DetID & (1 << i)) != 0)
                    ulGKEY_DetID &= ~(0x01<<i);    // GPIO ACTIVE LOW, Inverter
                else
                    ulGKEY_DetID |= (0x01<<i);    // GPIO ACTIVE LOW, Inverter
            }
...
//------------------------------------------------------------------------------
void GKEY_SetDetPin(uint8_t ubGpioNum, uint8_t ubPolar)
{
	if(ubGpioNum < GKEY_MAX_NUM)
    {   
        ulGKEY_DetEnFlag |= 1<<ubGpioNum;
        ulGKEY_Polar |= ubPolar<<ubGpioNum;
    }
}
#endif
