...
void AKEY_Init(void)
{
	...
	tAKEY_AinCh1 = SADC_CH3;
	SADC_Init();
	SADC_SetPinMode(tAKEY_AinCh1, SADC_PIN_ANALOG_INPUT_MODE);
	SADC_Enable();
}
//------------------------------------------------------------------------------
...
AKEY_ID_t AKEY_Detection(void)
{
	...
	//SADC
	uwAKEY_DetLvl = uwSADC_GetReport(tAKEY_AinCh1);
	if(uwAKEY_DetLvl >= tAKEY_Map[AKEY_UNKNOW].uwKeyLvl)
		return AKEY_UNKNOW;

	ubAKEY_MaxID = sizeof tAKEY_Map / sizeof(AKEY_MAP_t);
	for(ubAKEY_Idx = AKEY_POWER; ubAKEY_Idx < ubAKEY_MaxID; ubAKEY_Idx++)
	{
