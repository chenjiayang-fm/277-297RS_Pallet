...
void msc_card_in_task(void const *pvParameters)
{
	...
	for( ;; )
	{
...
#endif		
		{
				...
				MSC_DBG("msc_card_in_task : lun %d capactiy  = %llu !!", card_in_lun, msc_dev->msc_lun_info[card_in_lun].lun_capacity);
...
void msc_rw_test_task(void const *pvParameters)
{	
	uint8_t device_id = 0;
...
#if defined (CONFIG_USBH_CMSIS_OS)		
	osThreadDef(MSC_CARD_IN_TASK, msc_card_in_task, osPriorityNormal, 1, 256);	
...
void msc_rw_test(uint8_t device_id)
{
	...
	osThreadDef(task_name, msc_rw_test_task, osPriorityNormal, 1, 256);		
...
void msc_get_device_task(void const *pvParameters)
{
	uint8_t	cmd_type = 0;
	uint8_t device_id = 0;
...
{		
	...
	osThreadDef(MSC_GET_DEVICE_TASK, msc_get_device_task, osPriorityNormal, 1, 256);		
