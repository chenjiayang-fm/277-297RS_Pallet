...
int32_t cmd_system_date(int argc, char* argv[])
{
	...
	RTC_PCF85063A_Calendar_t tSys_Calendar;
#endif
	...
	if (OPT == 0) {
...
#elif(BSP_RTC_TIMER_SEL==RTC_TIMER_EXTERNAL)
		RTC_PCF85063A_GetCalendar(&tSys_Calendar);
#endif
		printf("===================================\n");
		...
		RTC_GetCalendar(&tSys_Calendar);
#elif(BSP_RTC_TIMER_SEL==RTC_TIMER_EXTERNAL)
		RTC_PCF85063A_SetCalendar(&tSys_Calendar);
		RTC_PCF85063A_GetCalendar(&tSys_Calendar);
#endif
		printf("===================================\n");
...
int32_t cmd_system_phymem_rw(int argc, char* argv[]) {
	...
	if (argc == 2)	// READ: phymem_rw address
	{
		addr = strtoul(argv[1], NULL, 0) & (~3);
		printf("===================================\n");
		if( addr>=0x91100350 && addr<=0x911003B8 )
		{
			printf("ASIC Sig-Del ADC Reg read\n");
			addr = 0x91100350;
			for (i=0; i<27; i+=1)
			{
				if( (i%4) ==0 )
					printf("Reg[0x%08X]:",addr + i*4);
				
				ulTemp = *((uint8_t*)addr + i*4);
				ulTemp = *((uint8_t*)addr + i*4);
				printf("  0x%08X",ulTemp);
	...
	else if (argc == 3)	// WRITE: phymem_rw address value
	{
		addr = strtoul(argv[1], NULL, 0) & (~3);
	...
	else if (argc == 4)	// Continuous read ASIC register n items
	{
		addr = strtoul(argv[1], NULL, 0) & (~3);
		if( addr>=0x91100350 && addr<=0x911003B8 )
		{
	...
	else if (argc == 5)	// write ASIC register with mask and base
	{
		if (strtoul(argv[3], NULL, 0) == 32) {
			addr = strtoul(argv[1], NULL, 0) & (~3);
