...
pvCLIRedirectCbFunc pCLI_RedirectCbFunc = NULL;
...
{
	pCLI_RedirectCbFunc = pCLIRedirectCbFunc;
}
//------------------------------------------------------------------------------
...
{
	int32_t iCLI_sPRfLvl = (int32_t)atoi(argv[1]);

	if((argc > 2) || (iCLI_sPRfLvl > 8))
		return cliFAIL;
	sPRF_SetDbgMsgLvl(iCLI_sPRfLvl);
	return cliPASS;
}
//------------------------------------------------------------------------------
...
int32_t CLI_parse_cmd(char *cmd, int isTAB)
{
	...
	if (pCLI_RedirectCbFunc != NULL)
	{
		pCLI_RedirectCbFunc(cmd);
...
{
#if defined(CONFIG_CLI_CMD_RTC676x)
	if (APP_GetRfTestMode() == APP_RF_TEST_ON)
	{
		printf("RF Test mode, using \"exit\" command to leave mode\n");
		CLI_register_redirect_cb_func(RTC676x_TestPutUartString);
		return;
	}
#endif
...
void CLI_Init()
{
	...
	CLI_recvInit();
	osThreadDef(cliThread, CLI_RecvThread, osPriorityBelowNormal, 1, 8192);
