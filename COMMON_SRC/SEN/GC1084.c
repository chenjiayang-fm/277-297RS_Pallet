/*!
	...
	\file		GC1084.c
	\brief		Sensor GC1084 relation function
	\author		BoCun
	\version	3.2
	\date		2020-09-09
...
static uint8_t regValTable[25][6] ={
...
//vts = 750, hts=1600                
//window 1280x720                    
//row time=44.444us,frmae rate=30fps 
//bayer order  GRBG                  
	0x82,0x03,0xfe,0xf0,                 
	0x82,0x03,0xfe,0xf0,                 
	0x82,0x03,0xfe,0xf0,                 
...
void SEN_PclkSetting(uint8_t ubPclkIdx)
{
    ...
    tAE_SensorPara.ulSensorPclk = 36000000;  
    tAE_SensorPara.ulSensorPixelPerLine = 1600;
...
uint8_t ubSEN_CheckSensorState(void)
{
		...
		bSEN_I2C_Read (GC1084_CHIP_ID_HIGH_ADDR, &pBuf[1]);
		bSEN_I2C_Read (GC1084_CHIP_ID_LOW_ADDR, &pBuf[0]);
		if (GC1084_CHIP_ID != uwPID)
		{
			printd(DBG_ErrorLvl, "Read chip ID fail.<0x%x 0x%x>\n", GC1084_CHIP_ID, uwPID);
					return 0;
		}
		return 1;
}
...
uint8_t ubSEN_SetSensorInitTable(void)
{
		...
		if(ubSEN_CheckSensorState() != 1)
				return 0;
		printd(DBG_CriticalLvl, "GC1084 Sensor\n");
...
{   
		bSEN_I2C_WriteTry(GC1084_FRAME_H, (uint8_t)((uwDL>>8) & 0xff), TRY_COUNTS);
		bSEN_I2C_WriteTry(GC1084_FRAME_L, (uint8_t)(uwDL & 0xff), TRY_COUNTS);	
}
...
{
	  bSEN_I2C_WriteTry(GC1084_EXP_H, (uint8_t)((uwExpLine>>8) & 0x3f), TRY_COUNTS);
	  bSEN_I2C_WriteTry(GC1084_EXP_L, (uint8_t)(uwExpLine & 0xff), TRY_COUNTS);	
}
...
void SEN_WrGain(uint32_t ulGainX1024)
{
		...
		bSEN_I2C_Write(0x00,0xd1,regValTable[i][0]);
		bSEN_I2C_Write(0x00,0xd0,regValTable[i][1]);
		bSEN_I2C_Write(0x03,0x1d,0x2e);
		bSEN_I2C_Write(0x0d,0xc1,regValTable[i][2]);
		bSEN_I2C_Write(0x03,0x1d,0x28);
		bSEN_I2C_Write(0x00,0xb8,regValTable[i][3]);
		bSEN_I2C_Write(0x00,0xb9,regValTable[i][4]);
		bSEN_I2C_Write(0x01,0x55,regValTable[i][5]);

		bSEN_I2C_Write(0x00,0xb1,(tol_dig_gain>>6));
		bSEN_I2C_Write(0x00,0xb2,((tol_dig_gain&0x3f)<<2));
...
void SEN_SetMirrorFlip(uint8_t ubMirrorEn, uint8_t ubFlipEn)
{
    ...
    if(ubMirrorEn)
        xtSENInst.ubImgMode |=  GC1084_MIRROR;
    else
				xtSENInst.ubImgMode &=  ~GC1084_MIRROR;
    ...
    if(ubFlipEn)
        xtSENInst.ubImgMode |=  GC1084_FLIP;
    else
        xtSENInst.ubImgMode &=  ~GC1084_FLIP;
...
{
    sensor_cfg.ubSensorType = SEN_GC1084;    

    printd(DBG_Debug1Lvl, "sensor type is GC1084\n");
...
void SEN_SetSensorEnvironment(uint32_t ulExp, uint32_t ulGain)
{
		...
		//EXPLINE
		bSEN_I2C_WriteTry(GC1084_EXP_H, (uint8_t)((ulExp>>8) & 0xff), TRY_COUNTS);
		bSEN_I2C_WriteTry(GC1084_EXP_L, (uint8_t)(ulExp & 0xff), TRY_COUNTS);  
