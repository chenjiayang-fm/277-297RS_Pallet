/*!
	...
	\file		H65.c
	\brief		Sensor H65 relation function
	\author		BoCun
	\version	3.2
	\date		2020-09-09
...
const uint8_t ubSEN_InitTable[] = {
	...
	// Initial Table
	//------------------------------
    // H65_24Minput_30fps_43MPclk_720p
...
uint8_t ubSEN_CheckSensorState(void)
{
	...
	bSEN_I2C_Read (H65_CHIP_ID_HIGH_ADDR, &pBuf[1]);
	bSEN_I2C_Read (H65_CHIP_ID_LOW_ADDR, &pBuf[0]);
	if (H65_CHIP_ID != uwPID)
	{
...
//	uint8_t     *pBuf;
    ...
    if(ubSEN_CheckSensorState() != 1)
        return 0;
    printd(DBG_CriticalLvl, "H65 Sensor\n");
...
{   
	//Set dummy lines
	bSEN_I2C_WriteTry(0xc6, H65_FRAME_H, TRY_COUNTS);
	bSEN_I2C_WriteTry(0xc7, (uint8_t)((uwDL>>8) & 0xff), TRY_COUNTS);
	TIMER_Delay_us(20);
	bSEN_I2C_WriteTry(0xc8, H65_FRAME_L, TRY_COUNTS);
...
void SEN_WrExpLine(uint16_t uwExpLine)
{
	...
	if(uwExpLine == uwOldExpline)
		return;
	bSEN_I2C_WriteTry(0xc0, H65_EXP_H, TRY_COUNTS);
	bSEN_I2C_WriteTry(0xc1, (uint8_t)((uwExpLine>>8) & 0xff), TRY_COUNTS);
	
	bSEN_I2C_WriteTry(0xc2, H65_EXP_L, TRY_COUNTS);
...
void SEN_WrGain(uint32_t ulGainX1024)
{
	...
	// gain	
	bSEN_I2C_WriteTry(0xc4, H65_GAIN, TRY_COUNTS);
...
void SEN_SetMirrorFlip(uint8_t ubMirrorEn, uint8_t ubFlipEn)
{
    ...
    if(ubMirrorEn)
        xtSENInst.ubImgMode |=  H65_MIRROR;
    else
        xtSENInst.ubImgMode &=  ~H65_MIRROR;
    
    if(ubFlipEn)
        xtSENInst.ubImgMode |=  H65_FLIP;
    else
        xtSENInst.ubImgMode &=  ~H65_FLIP;
...
{
    sensor_cfg.ubSensorType = SEN_H65;
    printd(DBG_Debug1Lvl, "sensor type is H65\n");	
}
