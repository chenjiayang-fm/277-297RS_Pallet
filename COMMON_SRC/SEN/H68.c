/*!
	...
	\file		H68.c
	\brief		Sensor H68 relation function
	\author		wenwei
	\version	4.0
	\date		2022-09-26
...
const uint8_t ubSEN_InitTable[] = {
    ...
    // Initial Table
    //------------------------------
    // H68_24Minput_30fps_43MPclk_720p
...
const uint8_t ubSEN_InitTable[] = {
	...
	0x82, 0x58, 0x18,
	0x82, 0x57, 0x60,
    // FrameW = (0x21<<8 + 0x20) << 1 =1920
    // FrameH = (0x23<<8 + 0x22)  = 750
	0x82, 0x20, 0xC0,
	0x82, 0x21, 0x03,
	0x82, 0x22, 0xEE,
	0x82, 0x23, 0x02,
    // Hwin = (0x26[3:0] << 8 + 0x24) = 640 << 1 =1280
    // Vwin = (0x26[7:4] << 8 + 0x25) = 720
	0x82, 0x24, 0x80,
	0x82, 0x25, 0xD0,
	0x82, 0x26, 0x22,
    // HwinS = (0x29[3:0] << 8) + 0x27 
    // VwinS = (0x29[7:4] << 8) + 0x28 
	0x82, 0x27, 0x3A,
	0x82, 0x28, 0x15,
	0x82, 0x29, 0x01,
	0x82, 0x2A, 0x2E,
...
uint8_t ubSEN_CheckSensorState(void)
{
	...
	bSEN_I2C_Read (H68_CHIP_ID_HIGH_ADDR, &pBuf[1]);
	bSEN_I2C_Read (H68_CHIP_ID_LOW_ADDR, &pBuf[0]);
	if (H68_CHIP_ID != uwPID)
	{
...
uint8_t ubSEN_SetSensorInitTable(void)
{
    ...
    if(ubSEN_CheckSensorState() != 1)
        return 0;
    printd(DBG_CriticalLvl, "H68 Sensor\n");
    //stream off
...
uint8_t ubSEN_Open(struct SENSOR_SETTING *setting)
{
    ...
    if (ubSEN_SetSensorInitTable() != 1)
    {
        printf("H68_MIPI startup failed! \n\r");
        return 0;
    }
...
{   
	//Set dummy lines
	bSEN_I2C_WriteTry(0xc6, H68_FRAME_H, TRY_COUNTS);
	bSEN_I2C_WriteTry(0xc7, (uint8_t)((uwDL>>8) & 0xff), TRY_COUNTS);
	TIMER_Delay_us(20);
	bSEN_I2C_WriteTry(0xc8, H68_FRAME_L, TRY_COUNTS);
...
void SEN_WrExpLine(uint16_t uwExpLine)
{
	...
	if(uwExpLine == uwOldExpline)
		return;
	bSEN_I2C_WriteTry(0xc0, H68_EXP_H, TRY_COUNTS);
	bSEN_I2C_WriteTry(0xc1, (uint8_t)((uwExpLine>>8) & 0xff), TRY_COUNTS);
	
	bSEN_I2C_WriteTry(0xc2, H68_EXP_L, TRY_COUNTS);
...
void SEN_WrGain(uint32_t ulGainX1024)
{
	...
	// gain	
	bSEN_I2C_WriteTry(0xc4, H68_GAIN, TRY_COUNTS);
...
void SEN_SetMirrorFlip(uint8_t ubMirrorEn, uint8_t ubFlipEn)
{
    ...
    if(ubMirrorEn)
        xtSENInst.ubImgMode |=  H68_MIRROR;
    else
        xtSENInst.ubImgMode &=  ~H68_MIRROR;
    
    if(ubFlipEn)
        xtSENInst.ubImgMode |=  H68_FLIP;
    else
        xtSENInst.ubImgMode &=  ~H68_FLIP;
...
{
    sensor_cfg.ubSensorType = SEN_H68;
    printd(DBG_Debug1Lvl, "sensor type is H68\n");	
}
