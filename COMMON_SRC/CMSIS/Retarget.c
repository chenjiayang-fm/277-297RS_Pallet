...
static UART1_Type *ptRETARGET_Uart = UART1;
//------------------------------------------------------------------------------
void RETARGET_Init (UART1_Type *ptUart)
{
	ptRETARGET_Uart = ptUart;
}
//------------------------------------------------------------------------------
...
int sendchar(int ch)
{
	if(ch == '\n')
	{
		while(!ptRETARGET_Uart->TX_RDY);
		ptRETARGET_Uart->RS_DATA = '\r';
	}
	else if(ch == '\r')
	{
		while(!ptRETARGET_Uart->TX_RDY);
		ptRETARGET_Uart->RS_DATA = '\n';
	}
	while(!ptRETARGET_Uart->TX_RDY);
	ptRETARGET_Uart->RS_DATA = ch;
	return ch;
}
//------------------------------------------------------------------------------
int getkey(void)
{
	...
	while(!ptRETARGET_Uart->RX_RDY);
	iRet = ptRETARGET_Uart->RS_DATA & 0x0FF;
