...
#define TIMER_MINORVER	7

//------------------------------------------------------------------------------
const uint32_t ulTimer_DeviceAddr[] = {Timer1_BASE, Timer1_BASE, Timer1_BASE,
									   Timer2_BASE, Timer2_BASE, Timer2_BASE};
const INTC_IrqSrc_t iTimer_IRQn[] = {INTC_TIMER1_IRQ, INTC_TIMER1_IRQ, INTC_TIMER1_IRQ,
									 INTC_TIMER2_IRQ, INTC_TIMER2_IRQ, INTC_TIMER2_IRQ};
const INTC_IrqHandler iTimer_IrqHandler[] = {TIMER1_INT_Handler, TIMER1_INT_Handler, TIMER1_INT_Handler,
...
void TIMER_Init(void)
{
	...
	for(ubIdx = 0; ubIdx < TIMER_TOTAL_NUM; ubIdx++)
	{
		...
		INTC_IrqSetup(iTimer_IRQn[(TIMER_DEVICE_t)(TIMER1_1 + ubIdx)], iTimer_IrqHandler[(TIMER_DEVICE_t)(TIMER1_1 + ubIdx)]);
		INTC_IrqEnable(iTimer_IRQn[(TIMER_DEVICE_t)(TIMER1_1 + ubIdx)]);
	}
#ifdef RTOS
	osMutexDef(TIMER32_delayMutex);
	TIMER32_delayMutex = osMutexCreate(osMutex(TIMER32_delayMutex));
#endif
}
//------------------------------------------------------------------------------
...
{
#ifdef RTOS
	osMutexWait(TIMER32_delayMutex, osWaitForever);
#endif
	Timer3->TM2_CLOCK 	= TIMER_CLK_EXTCLK;
	Timer3->TM2_LOAD 	= 10 * uwUs;							//!< Donw count from TM3_LOAD @ 10MHz clock
	Timer3->TM2_COUNTER = Timer3->TM2_LOAD;
	Timer3->TM2_MATCH1 	= Timer3->TM2_LOAD + 1;					//!< never reach to this match value
	Timer3->TM2_MATCH2 	= Timer3->TM2_LOAD + 1;					//!< never reach to this match value

	Timer3->TM2_UPDOWN 	= TIMER_DOWN_CNT;						//!< Down count
	Timer3->TM2_OF_EN 	= TIMER_OF_ENABLE;						//!< Enable underflow interrupt11
	Timer3->TM2_OF_MSK 	= 1;
	Timer3->TM2_EN 		= 1;
	while(!Timer3->TM2_OF_FLAG);
	Timer3->TM2_EN 		= 0;
	Timer3->CLR_TM2_OF_FLAG = 1;
#ifdef RTOS
	osMutexRelease(TIMER32_delayMutex);
#endif
}
//------------------------------------------------------------------------------
...
#endif
	Timer3->TM2_CLOCK 	= TIMER_CLK_EXTCLK;
	Timer3->TM2_LOAD 	= 10000 * uwMs;							//!< Donw count from TM3_LOAD @ 10MHz clock
	Timer3->TM2_COUNTER = Timer3->TM2_LOAD;
	Timer3->TM2_MATCH1 	= Timer3->TM2_LOAD + 1;					//!< never reach to this match value
	Timer3->TM2_MATCH2 	= Timer3->TM2_LOAD + 1;					//!< never reach to this match value
...
void TIMER_SetValue_us(TIMER_DEVICE_t tDevice,uint16_t uwUs)
{
	TIMER_DEV->TM2_CLOCK 	= TIMER_CLK_EXTCLK;
	TIMER_DEV->TM2_LOAD 	= uwUs;							//!< Donw count from TM3_LOAD @ 10MHz clock
	TIMER_DEV->TM2_COUNTER = TIMER_DEV->TM2_LOAD;
	TIMER_DEV->TM2_MATCH1 	= 0xfffffff;					//!< never reach to this match value
	TIMER_DEV->TM2_MATCH2 	= 0xfffffff;					//!< never reach to this match value
	TIMER_DEV->TM2_UPDOWN 	= TIMER_UP_CNT;						//!< Down count
	TIMER_DEV->TM2_OF_EN 	= TIMER_OF_DISABLE;						//!< Enable underflow interrupt11
	TIMER_DEV->TM2_OF_MSK 	= 0;
	TIMER_DEV->TM2_EN 		= 1;
}

uint16_t TIMER_GetValue_us(TIMER_DEVICE_t tDevice)
{
	uint16_t val = TIMER_DEV->TM2_COUNTER/10;
	return val;
}
