/*!
	...
	\file		LCD_YN700WS008B.c
	\brief		LCD YN700WS008B Funcation
	\author		Pierce
	\version	0.2
	\date		2020/06/01
...
void LCD_LVDS_YN700WS008B (void)
{
	//! TM035KDH03
	printf("LCD YN700WS008B\n");
	printf("24 Den Mode\n");
	printf("Screen Size 1024 x 600\n");			
	LCD->LCD_MODE = LCD_DE;
	LCD->SEL_TV = 0;								
	
	LCD->LCD_HO_SIZE = 1024;
	LCD->LCD_VO_SIZE = 600;	
	//! Timing
	LCD->LCD_HT_DM_SIZE = 16 + 80;
	LCD->LCD_VT_DM_SIZE = 1 + 20;
	LCD->LCD_HT_START = 1 + 159;
	LCD->LCD_VT_START = 1 + 22;
...
//==================================================================================	
	...
	LCD->LCD_PCK_SPEED = 2;//3;
	GLB->LCDPLL_CK_SEL = 1;//2;
	//! LCD PLL
	if (!GLB->LCDPLL_PD_N)
	{
		GLB->LCDPLL_LDO_EN = 1;
	...
	GLB->LCDPLL_INT = 12;
	GLB->LCDPLL_FRA = 0.48/*0.3648*/ * (1 << 20);
	TIMER_Delay_us(1);
	GLB->LCDPLL_INT_FRA_VLD = 1;
	TIMER_Delay_us(1);	
	//! LCD Controller rate
