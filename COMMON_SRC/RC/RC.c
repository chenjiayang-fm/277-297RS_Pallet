...
void RC_DynamicFpsProcess(uint8_t ubCodecIdx)
{
	...
	if((ubKNL_GetRtCommLinkStatus(KNL_MASTER_AP)) && (ubKNL_GetCommLinkStatus(KNL_MASTER_AP)) && (tRC_GetSysStatus()==RC_STATUS_RDY))
	{
		...
		{
			ulCurTargetDr = (ulAvgCurBw*7.5)/10;
		}
		else if(tRC_Info[ubCodecIdx].ubKeySecRatio == RC_RATIO_80)
		{
			ulCurTargetDr = (ulAvgCurBw*8)/10;
...
RC_TxBps_t tRC_sPRfTxBps[] = {//!					                    1T             2T             4T
								...
								{30,  43,  {30, 32, 32, 38, 30},  {(440*1024*8L), (220*1024*8L), (180*1024*8L)},  {RC_RATIO_70, RC_RATIO_60, RC_RATIO_60, RC_RATIO_40, RC_RATIO_30},  {22, 22, 22}},
								{45,  48,  {35, 37, 35, 41, 35},  {(330*1024*8L), (160*1024*8L), (150*1024*8L)},  {RC_RATIO_70, RC_RATIO_50, RC_RATIO_60, RC_RATIO_40, RC_RATIO_30},  {19, 19, 19}},
								{60,  51,  {41, 39, 42, 43, 40},  {(240*1024*8L), (120*1024*8L), (120*1024*8L)},  {RC_RATIO_70, RC_RATIO_50, RC_RATIO_50, RC_RATIO_40, RC_RATIO_30},  {16, 16, 16}},
								{75,  51,  {43, 41, 43, 45, 43},  {(160*1024*8L), (100*1024*8L), (100*1024*8L)},  {RC_RATIO_50, RC_RATIO_50, RC_RATIO_50, RC_RATIO_40, RC_RATIO_30},  {13, 13, 13}},
								{100, 51,  {45, 45, 46, 46, 45},  {(100*1024*8L), ( 80*1024*8L), ( 80*1024*8L)},  {RC_RATIO_50, RC_RATIO_50, RC_RATIO_50, RC_RATIO_40, RC_RATIO_30},  {10, 10, 10}},
							 };
static uint8_t ubRC_TxTarget = 2;
static uint8_t ubRC_TxFpsLvl = 0;
void RC_UpdateDynaFpsParam(uint8_t ubMaxFps, uint8_t ubMinFps)
{
	uint8_t ubLvl = 0;
	uint8_t ubFpsIntv = 0;
	uint8_t ubTxRateMd = 0;

	tRC_sPRfTxFps[ubTxRateMd].ubTargetFps[0] = ubMaxFps;
	tRC_sPRfTxFps[ubTxRateMd].ubTargetFps[DYNA_FPS_RC_LVL-1] = ubMinFps;
	ubFpsIntv = (uint8_t)(((float)((ubMaxFps - ubMinFps)))/(DYNA_FPS_RC_LVL - 1) + (float)0.5);
	for(ubLvl = 1; ubLvl < (DYNA_FPS_RC_LVL - 1); ubLvl++)
		tRC_sPRfTxFps[ubTxRateMd].ubTargetFps[ubLvl] = tRC_sPRfTxFps[ubTxRateMd].ubTargetFps[ubLvl-1] - ubFpsIntv;
	ubRC_CurFps = ubMaxFps - 3;
}
void RC_UpdateTrxBufParam(uint8_t ubToltalBufNum)
{
	uint8_t ubBpsLvlCnt = 6;
	uint8_t ubLvl, ubTxNum, ubTxRateMd = 0;
	uint8_t ubMaxBufNum = 30, ubMinBufNum = 10, ubBufNumIntv;

	ubBpsLvlCnt  = sizeof tRC_sPRfTxBps / sizeof(RC_TxBps_t);
	ubMaxBufNum  = ubToltalBufNum - 4;
	ubMinBufNum  = ubToltalBufNum * 0.6;
	...
	for(ubLvl = 0; ubLvl < 2; ubLvl++)
		tRC_sPRfTxFps[ubTxRateMd].ubBufNum[ubLvl] = ubMaxBufNum - ubLvl;
	for(;ubLvl < DYNA_FPS_RC_LVL; ubLvl++)
		tRC_sPRfTxFps[ubTxRateMd].ubBufNum[ubLvl] = tRC_sPRfTxFps[ubTxRateMd].ubBufNum[ubLvl-1] - ubBufNumIntv;
}
uint8_t ubRC_ChkAdjQpRange(uint8_t ubCodecIdx)
{
	sPRF_DevId_t tTxId;
	uint8_t ubBpsLvlCnt = sizeof tRC_sPRfTxBps / sizeof(RC_TxBps_t), ubRC_TxNum = 1;
	uint8_t ubRC_TxPer = 100, ubPerTarget = 3, ubLvl = 0;
	uint8_t ubRC_FreeBufNum = 0, ubBufTarget = 3;
	uint8_t ubCurTarget = 3, ubRC_TxMdSel = 0;
	   ...
	   (sPRF_DEVDRV_EN == tsPRF_GetDrvSts(tTxId)) && (sPRF_TRX_MODE == tsPRF_GetDrvMode()))
	{
		...
		if(ubRC_TxPer > 100)
			ubRC_TxPer = 100;
		if((ubRC_TxPer < 30) && (ubRC_FreeBufNum >= tRC_sPRfTxBps[0].ubBufNum[ubRC_TxMdSel-1]))
			return ubUpdFlag;
		...
		ubUpdFlag   = (ubCurTarget > ubRC_TxTarget)?TRUE:FALSE;
		if(TRUE == ubUpdFlag)
		{
			...
			ubRC_FinalMaxQp[ubCodecIdx] = (TRUE == ubLowLaty)?51:(tRC_sPRfTxBps[ubCurTarget].ubMaxQp + (((TRUE == ubSpecfyMd) && (ubCurTarget <= 2))?(2 - ubCurTarget):0));
			ubRC_FinalMinQp[ubCodecIdx] = tRC_sPRfTxBps[ubCurTarget].ubMinQp[ubQpLvl] + (((TRUE == ubSpecfyMd) && (ubCurTarget <= 2))?(4 - ubCurTarget):0);
			printd(DBG_CriticalLvl, "\n	Adj QP: %d->%d[%d][%d_%d][%d]\n", ubRC_TxTarget, ubCurTarget, ubRC_TxPer, ubRC_FinalMinQp[ubCodecIdx], ubRC_FinalMaxQp[ubCodecIdx], ubRC_FreeBufNum);
...
uint8_t ubRC_QtyAndDynaFpsProcess(uint8_t ubCodecIdx)
{
	   ...
	   (sPRF_DEVDRV_EN == tsPRF_GetDrvSts(tTxId)))
	{
		...
		for(ubLvl = 0; ubLvl < DYNA_FPS_RC_LVL; ubLvl++)
		{
			if(ubRC_FreeBufNum > tRC_sPRfTxFps[ubTxRateMd].ubBufNum[ubLvl])
			{
				if((ubRC_TxFpsLvl) && (ubLvl < ubRC_TxFpsLvl))
				{
					ubRC_TxFpsLvl -= 1;
					ubLvl = ubRC_TxFpsLvl;
				}
					...
					ubRC_TxFpsLvl = ubLvl;
				ubTargetFps = tRC_sPRfTxFps[ubTxRateMd].ubTargetFps[ubLvl];
				break;
			}
		}
		ubLowLaty  = (tRC_TxRateMd & 0x8)?TRUE:FALSE;
...
void RC_DtDynaQtyFpsProcess(uint8_t ubCodecIdx)
{
	...
	printd(DBG_CriticalLvl, "\nfps:%2d,%sQ:%2d,CQ:%2d,B:%2d,QP:%d,RSI[%3d],F=%02d Dg[%d %d %d/%d],BR[%03d/%d],Ps:%d/%d,Twc:%d/%d/%d,Hb:%d/%d,WT:%d,[%s]\n",
			ubCurFps, AvgFpsInfo,
			#if (APP_BLE_SN9380_FUNC_ENABLE == 1)
...
void RC_QtyAndBwModePreset(void)
{
	...
	if(DISPLAY_MODE == DISPLAY_1T1R)
		tRcInfo.ulInitBitRate	= RC_MAX_BW_A1_SERIES/2;
	if(DISPLAY_MODE == DISPLAY_2T1R)
		tRcInfo.ulInitBitRate	= RC_MAX_BW_A2_SERIES/2;
	if(DISPLAY_MODE == DISPLAY_4T1R)
		tRcInfo.ulInitBitRate	= RC_MAX_BW_A4_SERIES/2;
