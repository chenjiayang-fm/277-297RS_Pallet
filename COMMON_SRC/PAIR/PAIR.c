...
//		ulTemp = rand();
		...
		{
			srand(Timer3->TM3_COUNTER);
			ulTemp += rand();
			osDelay(10);
		}
		printf("11111(tPAIR_Info.ulPAIR_Sign != PAIR_SIGN) ulTemp = %u\n",ulTemp/10);
        PAIR_PapPket.ulAp_ID 	 = ulTemp/10;
	#ifdef A7130
    ...
    if(PAIR_PrpPket.ubTxNumber != PAIR_AP_SLAVE)
    {
		...
		printf("22222(PAIR_PrpPket.ubTxNumber != PAIR_AP_SLAVE) ulTemp = %u\n",ulTemp/10);
		PAIR_PapPket.ulSTA_ID[ubPAIR_Number]  = ulTemp/10;
