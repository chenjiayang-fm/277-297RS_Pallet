...
#if (defined(APP_USBD_UVCVDO_MODE) && ((APP_USBD_UVCVDO_MODE == 1)||(APP_USBD_UVCVDO_MODE == 2)||(APP_USBD_UVCVDO_MODE == 0)))
typedef enum
{
	KNL_USBD_FWU_MDA=0x0A,
	KNL_USBD_FWU_SDK=0x0B,			
}KNL_USBD_FWU_CMD;
typedef struct
{
	KNL_USBD_FWU_CMD		ubCmd;
}KNL_USBDFWU_PROCESS;
#endif
...
void KNL_BatteryInfoRptFunc(TWC_TAG GetSta, uint8_t *pData);
uint32_t ulKNL_GetBatteryInfoBwRpt(KNL_ROLE tRole);
void KNL_SetBatteryInfoBwRpt(KNL_ROLE tRole, uint8_t ubBatteryLvl);
void KNL_SetFlag4SkipFirstFrame(uint8_t ubSrcNum,uint8_t ubSkip);
...
\return(no)
*/
void KNL_SyncTimeStamp2TX(uint8_t ubEanble);
...
uint32_t KNL_GetFrameCount(KNL_ROLE tKNL_Role);
uint8_t KNL_GetFps(KNL_ROLE tKNL_Role);
//------------------------------------------------------------------------
/*!
...
//------------------------------------------------------------------------
uint8_t tKNL_GetPlayDestFld(void);
...
extern uint8_t tUI_RecCutOFF[4];
